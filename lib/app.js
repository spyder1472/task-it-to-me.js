      ...
      if (command === 'q') {
        ...
        if (command === 'c') {
          ...
          inputStream.once('data', function changeProjectName(rawData) {
            var oldName = data.currentProject;
          ...
          if (data.thereAreTasks()) {
            outputStream.write(messages.listTasks(data.currentProject, data.tasks()));
          } else {
            outputStream.write(messages.noTasksCreated(data.currentProject));
          }
          nextCommand();
        } else if (command === 'e') {
          outputStream.write(messages.promptForTaskName());
        ...
        if (command === 'ls') {
          if (data.thereAreProjects()) {
            outputStream.write(messages.listProjects(data.projectNames()));
            nextCommand();
          } else {
          ...
          if (data.thereAreProjects()) {
            ...
            inputStream.once('data', function deleteProjectIfExists(rawData) {
              var projectName = rawData.toString().trim();
              if (data.projectExists(projectName)) {
